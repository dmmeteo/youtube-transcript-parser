...
def get_transcript_for_video(video_id: str, language_code: str) -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            transcript = YouTubeTranscriptApi.get_transcript(
                video_id, languages=[auto_lang]
            )
            logger.debug(
                f"Found auto-generated transcript in language: {language_code}"
            )
            transcript_text = " ".join([seg["text"] for seg in transcript])
            return transcript_text
...
def create_dataset_entry(
    video_title: str, published_at: str, transcript: str
) -> Dict[str, Any]:
    """
...
def process_videos_streaming(
    videos: List[Dict], language_code: str, output_file: str
) -> int:
    """
            ...
            for video in tqdm(videos, desc="Processing videos", unit="video"):
                try:
                    ...
                    if transcript:
                        ...
                        logger.debug(
                            f"Transcript processed: {len(processed_transcript)} characters"
                        )
                    else:
...
def parse_arguments():
    ...
    parser.add_argument(
        "language_code", help="Language code for transcripts (e.g., 'en' for English)"
    )
    parser.add_argument(
        "--api-key",
...
def main():
    ...
    if not api_key:
        ...
        logger.error(
            "You can provide it with --api-key or set the YOUTUBE_API_KEY environment variable"
        )
        sys.exit(1)
    ...
    try:
        ...
        if not channel_id:
            ...
            logger.error(
                "Please use format: https://www.youtube.com/channel/CHANNEL_ID"
            )
            sys.exit(1)
    except Exception as e:
    ...
    try:
        ...
        processed_count = process_videos_streaming(
            videos, args.language_code, full_dataset_jsonl_path
        )
        ...
        if processed_count < 3:
            logger.warning(
                "Not enough data to create meaningful splits (minimum 3 entries required)"
            )
        else:
